...
// Exported (as a runtime enum) so consumers building their own source maps —
// e.g. the Volar integration — can tell the primary `full` tokens apart from
// the secondary `alias`/`anchor` tokens.
export enum Mapping {
  full,
  alias,
  anchor,
...
export class Extracted {
  ...
  // Exposed for the Volar integration, which reads the raw tokens to build
  // its own CodeMappings.
  tokens: Token[];
  constructor(
